...
class LastFmModule {
    ...
    fun provideLastFmService(): LastFmService {
        val lastFmInterceptor = Interceptor { chain ->
            ...
            val signedUrl = url.newBuilder()
                    .addQueryParameter("api_sig", url.calculateSign())
    ...
    @Provides
    fun provideApi(ktorApi: LastFmKtorApi): LastFmApi = ktorApi

    private fun HttpUrl.calculateSign(): String {
        return indexRange(querySize())
                .map { queryParameterName(it) to queryParameterValue(it) }
                .toMap()
                .calculateSign()
    }
